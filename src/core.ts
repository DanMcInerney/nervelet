...
import { bounded, boundedText, bytes, fail, message, now, stable } from './util.ts';
import type { Bundle, Command, ControlOutcome, Environment, Goal, GoalProvider, ImageAttachment, Limits, Receipt, Snapshot, StepOptions, StepRequest, Trace } from './types.ts';
import { instructions, RULE } from './instructions.ts';
...
type Delivery = { through: number; generation: number; goalVersion: number; resultIds:string[] };
...
export class Bridge {
  ...
  private receipts = new Map<string, { digest: string; result: Receipt; command: Command; acknowledged?:boolean }>();
  ...
  constructor(environment: Environment, goal?: string | Goal, options: BridgeOptions = {}) {
    ...
    this.profileText = instructions(environment.profile);
    ...
    if (bytes(this.profileText) + bytes(this.goal) + 512 > this.limits.maxRecoveryBytes) fail('capacity','Required recovery records cannot fit.');
  ...
  status() {
    return { epoch: this.epoch, loop: this.stopped ? 'stopped' : this.fault || this.transition || this.goal.status !== 'active' || this.refreshed !== this.generation ? 'paused' : 'active', goal: { ...this.goal }, generation: this.generation, ...(this.fault ? { fault: this.fault } : {}), ...(this.control ? {control:{...this.control}} : {}) };
  }
  ...
  private acknowledge(id?: string): void {
    ...
    this.emit({type:'acknowledgement',id});
  }
  async step(input: StepRequest = {}, signal?: AbortSignal, options: StepOptions = {}): Promise<Bundle> {
    ...
    try {
      ...
      if (commands.length) {
        ...
        await this.bundle(snapshot,[],false,input.schemaVersion === 2,options);
      ...
      if (snapshot.fault) this.fault = snapshot.fault;
      return await this.bundle(snapshot,results,true,input.schemaVersion === 2,options,{...media,...(wait ? {wait} : {})});
    } catch (error) { this.clearWait('step_failed'); throw error; }
    finally { this.inStep = false; }
  }
  private validateRequest(input: StepRequest, options: StepOptions): void {
    if (!validateStep(input) || bytes(input)>16384) fail('invalid_input','Invalid step request (including waitMs/wait syntax).');
    ...
    if ((input.commands?.length ?? 0)>this.limits.maxBatch || input.commands?.some(c => bytes(c)>4096)) fail('capacity','Command batch exceeds capacity.');
  ...
  private async admit(command: Command, signal?: AbortSignal): Promise<Receipt> {
    ...
    if (!validate || !validate(command.args)) result = {id:command.id,status:'rejected',reason:!validate?'unknown_command':'invalid_arguments'};
      ...
      try {
        ...
        result = await bounded(s => this.environment.execute(structuredClone(command),{signal:s,goalVersion,epoch:this.epoch,assertCurrent:() => {
          s.throwIfAborted();
    ...
    this.receipts.set(command.id,{digest,result:structuredClone(result),command:structuredClone(command)});
    this.emit({type:'admission',id:command.id,reason:result.status});
    return result;
  }
  async reconcile(id:string): Promise<Receipt> {
    const stored=this.receipts.get(id);
    if (!stored || stored.result.status !== 'unknown' || !this.environment.reconcile) fail('unsupported','No authoritative reconciliation is available.');
    const result=await bounded(s => this.environment.reconcile!(structuredClone(stored.command),s),this.limits.operationMs);
  ...
  private async snapshot(signal?: AbortSignal, after=this.through): Promise<Snapshot> {
    const snapshot=await bounded(s => this.environment.snapshot(after,s),this.limits.operationMs,signal);
  ...
  private async bundle(snapshot: Snapshot, results: Receipt[], deliver: boolean, v2=false, options:StepOptions={}, extra:Pick<Bundle,'attachments'|'media'|'wait'>={}): Promise<Bundle> {
    const generation = this.generation;
    const goalVersion=this.goal.version;
    ...
    const bundle: Bundle = {
      ...
      ...(v2 ? {schemaVersion:2 as const,loopRef:this.loopRef,generation,assembledMs:at} : {}),
    ...
    const textBytes=(b:Bundle) => bytes({...b,...(b.attachments ? {attachments:b.attachments.map(({data:_data,...metadata}) => metadata)} : {})});
    if (textBytes(bundle) + 64 > limit) fail('capacity','Current state exceeds bundle capacity; nothing was silently truncated.');
    ...
    for (const event of snapshot.events) {
      ...
      if (textBytes({...bundle,events:[...events,delivered],hasMore:true}) > limit) break;
    ...
    if (deliver) {
      this.sequence++;
      this.deliveries.set(bundle.id,{through,generation,goalVersion,resultIds:results.map(r=>r.id)});
      ...
      this.emit({type:'assembly',id:bundle.id,textBytes:textBytes(bundle),mediaBytes:extra.attachments?.reduce((sum,i) => sum+Buffer.byteLength(i.data,'base64'),0) ?? 0,inputs});
